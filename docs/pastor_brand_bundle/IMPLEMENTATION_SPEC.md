...
- **Vitest** for unit tests via Angular CLI’s experimental unit test runner (Angular docs show Vitest runner support):  
  https://v20.angular.dev/guide/testing/unit-tests
- **Playwright** for E2E (plus Playwright MCP for Copilot agent workflows):  
  https://github.com/executeautomation/mcp-playwright
- **Chrome DevTools MCP** for debugging/perf checks via agent tooling:  
  https://github.com/mcp/chromedevtools/chrome-devtools-mcp
- **Azure Static Web Apps** hosting + **GitHub Actions** deploy using `Azure/static-web-apps-deploy`:  
  https://github.com/Azure/static-web-apps-deploy
- **Terraform** provisioning Azure Static Web App (use `azurerm_static_web_app` or AVM module):  
  https://github.com/Azure/terraform-azurerm-avm-res-web-staticsite
...
Use Angular’s Tailwind integration (`ng add tailwindcss`) and keep Tailwind in `src/styles.css` as import:  
https://angular.dev/guide/tailwind

Also:
- Add `@tailwindcss/postcss` plugin if manual setup is needed (per Angular docs):  
https://angular.dev/guide/tailwind
...
- Update `angular.json` test target to `@angular/build:unit-test` with `"runner": "vitest"` as shown in Angular docs:  
https://v20.angular.dev/guide/testing/unit-tests
...
Install MCP server so Copilot can drive browsers in dev:  
https://github.com/executeautomation/mcp-playwright
...
Docs describe `chrome-devtools-mcp` as an MCP server exposing Chrome DevTools capabilities to coding agents:  
https://github.com/mcp/chromedevtools/chrome-devtools-mcp
...
### GitHub Action
Use the official action `Azure/static-web-apps-deploy` to build and deploy:  
https://github.com/Azure/static-web-apps-deploy

### Secret
Create repository secret:
- `AZURE_STATIC_WEB_APPS_API_TOKEN` (deployment token) per GitHub docs:  
https://docs.github.com/en/enterprise-server%403.15/actions/how-tos/deploy/deploy-to-third-party-platforms/azure-static-web-app
...
Use `azurerm_static_web_app` directly or AVM module for SWA; AVM module explicitly targets Static Web Apps:  
https://github.com/Azure/terraform-azurerm-avm-res-web-staticsite
...
✅ Unit tests run with Vitest via `ng test` config: https://v20.angular.dev/guide/testing/unit-tests  
✅ Playwright E2E tests cover route loading + invite form validation  
✅ Deploys to Azure Static Web Apps via GitHub Actions + token secret: https://docs.github.com/en/enterprise-server%403.15/actions/how-tos/deploy/deploy-to-third-party-platforms/azure-static-web-app
